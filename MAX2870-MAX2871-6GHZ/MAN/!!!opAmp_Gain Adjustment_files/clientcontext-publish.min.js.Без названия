...
if(s.method.toUpperCase()==="GET"||(t&&!j(t))){return
...
}}else{if(t=="select"){g.push({text:CQ.shared.Form.getLabelForField(q,l),value:p,name:p,enumeration:[],local:s,type:t,defaultValue:undefined,node:q});
v=g[g.length-1];
...
}function EditMode(){this.isEditMode=function(){return(CQ.WCM)?CQ.WCM.isEditMode(true):false
}
}var mode=new EditMode();
if(!mode.isEditMode()){Granite.I18n.setUrlPrefix("/libs/maxim/custom_i18n_fetch.")
};